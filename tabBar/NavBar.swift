...
class NavBar: UIView, LaunchMenuDelegate {
    ...
    var setViewWithItemIndex:Int {
        set{
            ...
            UIView.animate(withDuration: 0.2, animations: { () -> Void in
                ...
                self.line.frame = CGRect(x: itemBtn.frame.origin.x + 2, y: self.line.frame.origin.y, width: lineWidth - 4.0, height: self.line.frame.size.height)
                for btn in self.items {
                    let tempBtn = btn as! UIButton
                    tempBtn.setTitleColor(UIColor.white, for: UIControlState())
                    tempBtn.titleLabel?.font = UIFont.systemFont(ofSize: 16)
                }
                itemBtn.setTitleColor(Constants.news_title_desc, for: UIControlState())
                itemBtn.titleLabel?.font = UIFont.systemFont(ofSize: 18)
            }) 
        }
        get{
    ...
    fileprivate var skLaunchMenu:LaunchMenu!        //扩展菜单栏
    ...
    fileprivate func configView() {
        ...
        if showArrowButton {
            arrowBtnImageView = UIImageView(frame: CGRect(x: frameX, y: Constants.StatusHeight+10, width: 20, height: 20))
            ...
            let tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(NavBar.arrowBtnTapGesAction))
            arrowBtnImageView.addGestureRecognizer(tapGestureRecognizer)
        }
        scNavBar = UIScrollView(frame: CGRect(x: 0, y: 0, width: frameX, height: kScNavBarHeight+Constants.StatusHeight))
    ...
    fileprivate func getScNavContentAddScNavBarItemsWithItemsWidth(_ widths:NSArray) -> CGFloat {
        ...
        for index in 0 ..< itemsTitles.count {
            ...
            button.frame = CGRect(x: buttonX, y: Constants.StatusHeight, width: widths[index] as! CGFloat, height: kScNavBarHeight)
            button.setTitle((itemsTitles[index] as! String), for: UIControlState())
            button.setTitleColor(UIColor.white, for: UIControlState())
            button.titleLabel?.font = UIFont.systemFont(ofSize: 16)
            button.addTarget(self, action: #selector(NavBar.itemsBtnAction(_:)), for: UIControlEvents.touchUpInside)
        ...
        btn.setTitleColor(Constants.news_title_desc, for: UIControlState())
    ...
    fileprivate func setLineWithItensWidth(_ width:CGFloat) {
        line = UIView(frame: CGRect(x: 2, y: Constants.StatusHeight+kScNavBarHeight - 3, width: width - 7, height: 3))
...
//        view.layer.shadowOpacity = shadowOpacity
    ...
    fileprivate func getItemsWidthWithTitles(_ titles:NSArray) -> NSArray {
        ...
        for title in titles {
            ...
            let size:CGSize = str.size(attributes: [NSFontAttributeName:UIFont.systemFont(ofSize: UIFont.systemFontSize)])
                ...
                self.arrowBtnImageView.transform = CGAffineTransform(rotationAngle: CGFloat(M_PI))
                    ...
                    UIView.animate(withDuration: 0.2, animations: { () -> Void in
                        if self.skLaunchMenu == nil {
                            ...
                            self.skLaunchMenu = LaunchMenu(layout: layout, subViewTitles: self.itemsTitles)
