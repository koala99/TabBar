...
protocol LaunchMenuDelegate: NSObjectProtocol {
    
    func itemPressedWithIndex(_ index:Int)
}

class LaunchMenu: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
     ...
     */
    weak var delegate:LaunchMenuDelegate?
    ...
    var setLayout:UICollectionViewFlowLayout {
        set{
            ...
            layout = newValue
            layout.itemSize = CGSize(width: kScreenWidth / count, height: 40)
    ...
    fileprivate func createLaunchMenu() {
        ...
        launchMenu.register(LaunchMenuItem.self, forCellWithReuseIdentifier: "ITEM")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ITEM", for: indexPath) as! LaunchMenuItem
        let title = subViewTitles[(indexPath as NSIndexPath).row] as! String
        cell.titleLabel.text = title
        return cell
    }
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        delegate?.itemPressedWithIndex((indexPath as NSIndexPath).row)
    }
